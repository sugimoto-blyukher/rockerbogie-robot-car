...
/* [表示・出力コントロール] */
// 出力したいパーツを選択してください
part = "all"; // [all: 全て配置（確認用）, rocker: ロッカーアームのみ, bogie: ボギーアームのみ]

/* [共通パラメータ] */
$fn = 60;          // 円の分割数（精度）
link_h = 12;        // アームの厚み (mm)
pin_d = 4;         // シャフト・ネジ用の穴の直径 (mm) 
joint_d = 14;      // ジョイント結合部の外径 (mm) -
...
module rocker_arm() {
    // 各ピボットの座標定義
    p_front = [-50, -32.5];                     // フロントホイール軸
    p_body  = [70, 45];                   // 車体差動ギア接続ピボット
